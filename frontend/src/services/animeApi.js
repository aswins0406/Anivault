...
const BACKEND_URL =
  ...
  : (import.meta.env.VITE_BACKEND_URL || "https://anivault-backend-b6z3.onrender.com");
...
async function fetchBackend(path) {
  ...
  try {
    ...
    if (error.name === "AbortError") {
      throw new Error("Backend request timed out. Is port 10000 running?");
    }
...
export async function getTopAnime(page = 1) {
  return fetchBackend(`/api/anime/top?page=${Number(page) || 1}`);
}

export async function getTrendingAnime(page = 1) {
  return fetchBackend(`/api/anime/trending?page=${Number(page) || 1}`);
}

export async function getAnimeDetails(id) {
  const response = await fetchBackend(`/api/anime/${encodeURIComponent(id)}`);
...
export async function searchAnime(query, page = 1) {
  ...
  return fetchBackend(
    `/api/anime/search?q=${encodeURIComponent(cleanQuery)}&page=${Number(page) || 1}`
  );
}
